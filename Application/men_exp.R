...
target <- function(y) profile_loglik_cens_pwexp(p = c(0, y), df)
...
fit_alpha_1 <- mle_cens_pwexp(df, fit_p_1)
...
fit_alpha_2 <- mle_cens_pwexp(df, fit_p_2)
...
fit_alpha_3 <- mle_cens_pwexp(df, fit_p_3)

# AIC ---------------------------------------------------------------------

AIC_1 <- 2 * 3 - 2 * loglik_cens_pwexp(fit_alpha_1, df, fit_p_1)
AIC_2 <- 2 * 5 - 2 * loglik_cens_pwexp(fit_alpha_2, df, fit_p_2)
AIC_3 <- 2 * 7 - 2 * loglik_cens_pwexp(fit_alpha_3, df, fit_p_3)

# BIC ---------------------------------------------------------------------

BIC_1 <- log(n) * 3 - 2 * loglik_cens_pwexp(fit_alpha_1, df, fit_p_1)
BIC_2 <- log(n) * 5 - 2 * loglik_cens_pwexp(fit_alpha_2, df, fit_p_2)
BIC_3 <- log(n) * 7 - 2 * loglik_cens_pwexp(fit_alpha_3, df, fit_p_3)
...
p_df_x <- data.frame(p = fit_p_2,
                     e = S_ray_x(fit_p_2))

g_km_x <- 
  autoplot(KS_fit, conf.int.fill = 'dodgerblue3', conf.int.alpha = 0.1) + theme_bw() +
  stat_function(fun=S_exp_x, col = "dodgerblue3", size = 0.7) + 
...
ggsave(plot = g_km_x, "FITH.pdf", height = 6, width = 10)
